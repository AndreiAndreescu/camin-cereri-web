...
export default function Dashboard({ user }) {
  ...
  const [selectedProducts, setSelectedProducts] = useState({}); // { [productId]: cantitate }
  ...
  function setProductQty(id, value) {
    setSelectedProducts((s) => ({ ...s, [id]: value }));
  }
  ...
  async function onSubmitNewRequest(e) {
    ...
    const items = products
      ...
      .map((p) => ({ produs: p.name, cantitate: String(selectedProducts[p.id] || "").trim() }));
