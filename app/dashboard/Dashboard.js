      ...
      .map(
        (it) =>
          `<tr><td>${it.nr_crt}</td><td>${escapeHtml(it.produs)}</td><td>${escapeHtml(
            it.cantitate
          )}</td><td>${escapeHtml(it.detalii)}</td><td>${escapeHtml(it.culoare)}</td><td>${escapeHtml(
            it.marime
          )}</td><td>${escapeHtml(it.sex)}</td></tr>`
      )
      .join("");

    const html = `<!DOCTYPE html>
<html lang="ro"><head><meta charset="utf-8"><title>Cerere #${r.id}</title>
<style>
  ...
  <h1>Cerere #${r.id} — ${escapeHtml(centerName(r.center_id))}${r.urgent ? " (URGENT)" : ""}</h1>
  <p>Status: ${STATUS_LABELS[r.status] || r.status} · Creat de ${escapeHtml(r.created_by_name)} pe ${fmtDate(
      r.created_at
    )}</p>
  <table>
    <thead><tr><th>Nr. crt.</th><th>Produs</th><th>Cantitate</th><th>Detalii</th><th>Culoare</th><th>Mărime</th><th>Sex</th></tr></thead>
  ...
  function exportShoppingListCSV() {
    const header = ["Nr. crt.", "Produs", "Cantitate", "Detalii", "Culoare", "Mărime", "Sex"];
    ...
    <div id="app-screen">
      ...
      <main>
        ...
        {isAdmin && (
          <section className="panel">
            ...
            <p className="muted-note" style={{ marginTop: -8, marginBottom: 14 }}>
              ...
              cantitate, printre cererile acceptate momentan.
            </p>
        ...
        <section className="panel">
          ...
          <p className="muted-note" style={{ marginTop: -8, marginBottom: 14 }}>
            Catalogul din care se poate alege la o cerere nouă. Admin și administrator de centru pot
            adăuga și edita produse. Ștergerea (doar admin) nu afectează cererile deja trimise.
          </p>
