...
export default function Dashboard({ user }) {
  ...
  // panourile unul sub altul, fiecare panou e o pagina separata, aleasa din
  // meniul de sus, ca sa nu mai fie nevoie sa dai scroll ca sa gasesti ceva.
  ...
  const [attachmentUploading, setAttachmentUploading] = useState({}); // { [requestId]: bool }
  const [attachmentErrors, setAttachmentErrors] = useState({}); // { [requestId]: string }
  ...
  const [editingUser, setEditingUser] = useState({ full_name: "", email: "", role: "administrator_centru", center_ids: [], password: "" });
  ...
  // Cine poate atasa/sterge fisiere la un referat: admin, la orice referat,
  // sau administrator de centru, doar la referatele centrului/centrelor lui -
  // indiferent de status.
  function canManageAttachments(r) {
    if (isAdmin) return true;
    if (user.role === "administrator_centru") return userCenterIds.includes(r.center_id);
    return false;
  }
  ...
  // Meniul de sus - fiecare intrare e o "pagina" separata (un singur panou
  // vizibil odata). Numarul din paranteza ajuta sa vezi dintr-o privire daca
  // are rost sa intri intr-o pagina, fara sa mai trebuiasca sa dai click.
  const tabs = useMemo(
    () =>
      [
        { key: "referate", label: "Referate de Necesitate", count: requests.length, show: true },
        { key: "nou", label: "Referat nou", show: true },
        { key: "de-luat", label: "De luat", count: shoppingList.length, show: isAdmin },
        { key: "cautare", label: "Caută produs pe centre", show: isAdmin },
        { key: "produse", label: "Produse", count: products.length, show: true },
        { key: "centre", label: "Centre", count: centers.length, show: isAdmin },
        { key: "utilizatori", label: "Utilizatori", count: users.length, show: isAdmin },
      ].filter((t) => t.show),
    [requests.length, shoppingList.length, products.length, centers.length, users.length, isAdmin]
  );
  ...
  // Fisiere atasate (poze/documente) - functioneaza indiferent de statusul
  // referatului. Upload-ul foloseste FormData in loc de helper-ul api(), ca
  // sa lase browserul sa seteze singur Content-Type-ul (cu boundary).
  ...
  async function onDeleteAttachment(requestId, attachmentId) {
    ...
    try {
      await api(`/api/requests/${requestId}/attachments/${attachmentId}`, { method: "DELETE" });
      await loadRequests();
    } catch (err) {
      setAttachmentErrors((prev) => ({ ...prev, [requestId]: err.message }));
    }
                    ...
                    {r.attachments && r.attachments.length > 0 ? (
                      <ul className="attachments-list">
                        {r.attachments.map((a) => (
                          <li key={a.id}>
                            <a
                              href={`/api/requests/${r.id}/attachments/${a.id}`}
                              target="_blank"
