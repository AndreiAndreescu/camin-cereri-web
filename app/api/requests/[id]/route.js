...
export async function PATCH(request, { params }) {
  ...
  if (user.role === "admin") {
    ...
    if (!allowedCenters.includes(existing.center_id) || !allowedCenters.includes(Number(centerId))) {
      return NextResponse.json(
        { error: "Nu ai voie să muți referate de necesitate către/de la un centru care nu e al tău." },
        { status: 403 }
      );
    }
  } else {
...
export async function DELETE(request, { params }) {
  ...
  if (user.role === "admin") {
    ...
    if (!allowedCenters.includes(existing.center_id)) {
      return NextResponse.json(
        { error: "Nu ai voie să ștergi referate de necesitate de la alt centru." },
        { status: 403 }
      );
    }
  } else {
