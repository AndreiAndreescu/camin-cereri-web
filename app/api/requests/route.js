...
function mapRequest(row) {
  ...
  const attachments = (row.request_attachments || [])
    .slice()
  ...
  const { request_items, request_attachments, ...rest } = row;
...
export async function GET(request) {
  ...
  let query = supabaseAdmin()
    .from("requests")
    .select("*, request_items(*), request_attachments(*)")
    .order("created_at", { ascending: false });

  // Un administrator de centru vede DOAR cererile centrului/centrelor la
  // care e asignat, indiferent ce filtru trimite - restrictia se aplica
  // mereu pe server, nu doar in interfata.
  if (user.role === "administrator_centru") {
    ...
    if (centerId) {
      ...
      query = query.in("center_id", allowedCenters);
    }
  } else if (centerId) {
    query = query.eq("center_id", centerId);
  }
...
export async function POST(request) {
  ...
  if (user.role === "administrator_centru") {
    const allowedCenters = user.center_ids || [];
    if (!allowedCenters.includes(Number(centerId))) {
      return NextResponse.json({ error: "Nu ai voie sa creezi referate de necesitate pentru acest centru." }, { status: 403 });
    }
  }
