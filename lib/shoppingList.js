...
export function buildShoppingListByCenter(requests) {
    ...
    for (const it of r.items) {
      ...
      if (!parsed) {
        unmerged.push({
          ...
          detalii: detaliiDisplay,
          center_id: r.center_id,
        });
        continue;
      }
      const key =
        normalizeProductKey(it.produs) + "||" + normalizeDetailKey(it.detalii) + "||" + r.center_id;
      const displayName = it.produs.trim();
      if (!groups.has(key)) {
        groups.set(key, {
          produs: displayName,
  ...
  for (const u of unmerged) {
    rows.push({ produs: u.produs, cantitate: u.cantitate, detalii: u.detalii, center_id: u.center_id });
  }
